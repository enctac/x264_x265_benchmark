...
REM =================================================================================
REM ★注意
REM　●VCEEncに対応している環境と、rigaya氏のVCEEncCが必要です。
REM　　　http://rigaya34589.blog135.fc2.com/
REM　　からVCEEncをダウンロードし、その中のVCEEncCフォルダを
REM　　このバッチのある場所のbinフォルダの下に置いて下さい。
REM　●エンコード対象として指定できるのは、
REM　　VCEEncC.exe --avvceでハードウェアデコードできる動画ファイルのみです。
REM =================================================================================

REM ベンチマークのタイトル
set benchname=VCEEncC HEVC main VBR

REM 出力ファイル名の冒頭
set outname=VCEEncC_HEVC_main_VBR
...
set bindir=.\bin
set encoder=%bindir%\VCEEncC\x64\VCEEncC64.exe
IF NOT "%PROCESSOR_ARCHITECTURE%" == "AMD64" (
 set encoder=%bindir%\VCEEncC\x86\VCEEncC.exe
)
...
set encopt=--avvce --vbr %bitrate% --codec hevc --profile main
...
call :encode balanced "【Balanced】" FIRST
...
@echo.
@echo 　%benchname% --quality %1 %encopt% のエンコード中です...
...
%encoder% -i %inputfile% --quality %1 %encopt% -o %outfile% >> %outlog% 2>&1
